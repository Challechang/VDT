...
#         facesclassfier = cv2.CascadeClassifier("D:/opencv/opencv/sources/data/haarcascades/haarcascade_frontalface_default.xml")   #定义分类器
        ...
        Button(controlWindow, text="Switch Cur", width=8, fg=self.fontcolor, bg=self.widgetcolor,
               command=self.switchCurve).grid(row=1, column=0, pady=1)
        Button(controlWindow, text="Del UnSel", width=8, fg=self.fontcolor, bg=self.widgetcolor).grid(row=2, column=0, pady=1)
        Button(controlWindow, text="Cap Video", width=8, fg=self.fontcolor, bg=self.widgetcolor,
               command=self.capVideo).grid(row=3, column=0, pady=1)
...
#         perclosPlot.set_ylim(-1,1)
        ...
        updateCurveAll(self.master, canvas, figure)
